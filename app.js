...
async async function loadPlans() {
  if (!API_URL) return;
  try {
...
// Live-Sync: Änderungen, die IRON direkt am PC erstellt, werden automatisch
// auf den offenen Plans-/Tasks-Seiten angezeigt.
let liveSyncTimer = null;
function startLiveSync() {
  if (liveSyncTimer) return;
  liveSyncTimer = setInterval(() => {
    if (document.hidden) return;
    if (document.getElementById("plansList")) renderPlansScreen();
    if (document.getElementById("tasksList")) renderTasksScreen();
    // Falls die alte Übersicht noch vorhanden ist, ebenfalls aktualisieren.
...
// IRON v8: zentraler Live-Sync für PC -> Website
let ironLastPlanStamp = 0;
let ironLastTaskStamp = 0;
async function ironV8Sync() {
  if (!API_URL || document.hidden) return;
  try {
    const r = await fetch(`${API_URL}/api/sync?t=${Date.now()}`, {cache: "no-store"});
    if (!r.ok) return;
    const data = await r.json();
    const ps = Number(data.plan_updated_at || 0);
    const ts = Number(data.task_updated_at || 0);
    if (document.getElementById("plansList") && ps !== ironLastPlanStamp) {
      ironLastPlanStamp = ps;
      renderPlansScreen(data.plans || []);
    }
    if (document.getElementById("tasksList") && ts !== ironLastTaskStamp) {
      ironLastTaskStamp = ts;
      renderTasksScreen(data.tasks || []);
    }
  } catch (_) {}
}

// Ersetzt die Seiten-Funktionen durch Varianten, die Daten direkt aus /api/sync verwenden.
const ironV8RenderPlansScreen = window.renderPlansScreen;
const ironV8RenderTasksScreen = window.renderTasksScreen;
window.renderPlansScreen = async function(plansArg) {
  if (plansArg) return ironV8RenderPlansFromData(plansArg);
  return ironV8RenderPlansScreen();
};
function ironV8RenderPlansFromData(plans) {
  const el = document.getElementById("plansList");
  if (!el) return;
  el.innerHTML = plans.length ? plans.map(p => `<article class="data-card"><div class="data-head"><strong>${escapeHtml(p.titel || "Plan")}</strong><button onclick="deletePlanAndRefresh('${p.id}')">×</button></div><small>${escapeHtml(p.erstellt || "")}</small><div class="entries">${(p.eintraege||[]).map(x=>`<div><b>${escapeHtml(x.tag)}</b><span>${escapeHtml(x.inhalt)}</span></div>`).join("")}</div>${p.hinweis?`<p class="muted">${escapeHtml(p.hinweis)}</p>`:""}</article>`).join("") : `<div class="empty-screen"><div>◫</div><strong>NO PLANS</strong><p>Sag zu IRON zum Beispiel: „Erstelle einen Fitnessplan.“</p></div>`;
}
window.renderTasksScreen = async function(tasksArg) {
  if (tasksArg) return ironV8RenderTasksFromData(tasksArg);
  return ironV8RenderTasksScreen();
};
function ironV8RenderTasksFromData(tasks) {
  const el = document.getElementById("tasksList");
  if (!el) return;
  el.innerHTML = tasks.length ? tasks.map(t => `<article class="data-card task-card screen-task ${t.erledigt ? "done" : ""}"><label><input type="checkbox" ${t.erledigt?"checked":""} onchange="toggleTaskAndRefresh('${t.id}', this.checked)"><span>${escapeHtml(t.text || "Aufgabe")}</span></label><small>${escapeHtml(t.datum || "Offen")}</small></article>`).join("") : `<div class="empty-screen"><div>✓</div><strong>NO TASKS</strong><p>Sag zu IRON zum Beispiel: „Ich muss heute noch putzen.“</p></div>`;
}
setInterval(ironV8Sync, 1500);
document.addEventListener("visibilitychange", () => { if (!document.hidden) ironV8Sync(); });
ironV8Sync();

...
async function createPlan(name, inhalt){
  ...
  const row = await cloud.createRow(cloud.cfg.tables.plans, payload);
...
async function renderPlansScreen(){
  ...
  let rows=[];
  try{
    const data=await cloud.listRows(cloud.cfg.tables.plans);
  ...
  rows=rows
    .filter(r=>!String(r.name||"").startsWith("EINKAUF // "))
  ...
  host.querySelectorAll("[data-delete-plan]").forEach(btn=>{
    btn.onclick=async()=>{
      try{
        await cloud.deleteRow(cloud.cfg.tables.plans,btn.dataset.deletePlan);
...
async function renderShoppingScreen(){
  ...
  try{
    ...
    console.warn("Appwrite shopping read:",e);
  }
  ...
  if(!rows.length){
    host.innerHTML="<p>Noch keine Einkaufsliste gespeichert.</p>";
    return;
  }
  ...
  host.innerHTML=rows.map(r=>`
    <article class="saved-card">
      <header>
        <h3>${escapeHtml(String(r.name||"").replace(/^EINKAUF\s*\/\/\s*/,"") || "Einkaufsliste")}</h3>
        <small>${escapeHtml(formatDate(r.erstellt))}</small>
      </header>
      <pre>${escapeHtml(r.inhalt||"")}</pre>
      ${r.$id && !String(r.$id).startsWith("local_")
  ...
  host.querySelectorAll("[data-delete-shop]").forEach(btn=>{
    btn.onclick=async()=>{
      try{
        await cloud.deleteRow(cloud.cfg.tables.plans,btn.dataset.deleteShop);
        await renderShoppingScreen();
        await renderPlansScreen();
      }catch(e){ show(`Löschen fehlgeschlagen: ${e.message}`); }
    };
...
if(gmailBtn&&gmailModal) gmailBtn.onclick=()=>{gmailModal.hidden=false;};
